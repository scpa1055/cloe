...
JsonFileSerializer::~JsonFileSerializer() {}
...
std::unique_ptr<JsonFileSerializer> makeJsonFileSerializer(FileTypeEnum type, Logger log) {
  ...
  switch (type) {
    case JSON_BZIP2:
      result = std::unique_ptr<JsonFileSerializer>(std::make_unique<BZip2JsonSerializer>(log));
      break;
    case JSON_GZIP:
    ...
    case JSON:
      ...
      break;
    default:
      result = makeJsonFileSerializer(FileTypeEnum::JSON, log);
      break;
  }
  return result;
}
